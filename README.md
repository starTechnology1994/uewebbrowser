...
> ⚠️ **不要点击 GitHub 网页上的 "Code → Download ZIP" 下载源码**，该 zip 不包含 Git LFS 托管的大文件（CEF 二进制、预编译 payload），插件将无法加载。请从下面的 [Releases](https://github.com/starTechnology1994/uewebbrowser/releases) 下载完整插件包，或使用 `git lfs clone`。
>
> **项目主页与源码仓库**：[https://github.com/starTechnology1994/uewebbrowser](https://github.com/starTechnology1994/uewebbrowser)
>
> - 预编译插件下载：[https://github.com/starTechnology1994/uewebbrowser/releases](https://github.com/starTechnology1994/uewebbrowser/releases)
...
**Blueprint：** 在 Details 面板绑定 `OnMessageReceived` 事件，`FunctionName` 和 `MessageBody` 作为参数传入。
...
插件的 UE_LOG 输出（`LogWebNative` 类别）会自动写入独立日志文件：
...
| `gpu_shared_mutex_per_adapter` | bool | `false` | 按 GPU 适配器限制 shared_texture 并发数（`true`：每物理 GPU 仅 1 个实例 GPU 加速，其余 CPU 降级；`false`：所有实例均可 GPU 加速，纯 D3D12 拷贝路径下已无跨设备冲突，**推荐 `false`**）                                                                                                                                                                                                                                   |
...
| 发行版                       | GLIBC | 兼容状态 | 备注                              |
| ---------------------------- | ----- | -------- | --------------------------------- |
| Ubuntu 22.04 / 24.04         | 2.35+ | ✅ 支持  |                                   |
| Ubuntu 20.04                 | 2.31  | ✅ 支持  |                                   |
| Debian 11 (Bullseye)         | 2.31  | ✅ 支持  |                                   |
| Debian 12 (Bookworm)         | 2.36  | ✅ 支持  |                                   |
| CentOS Stream 9 / RHEL 9     | 2.34  | ✅ 支持  |                                   |
| CentOS 8 / RHEL 8            | 2.28  | ✅ 支持  | 已 EOL，建议升级                  |
| 麒麟 V10 SP1 (入门版/基础版) | 2.28  | ✅ 支持  | x86_64 版本，需确认 GLIBC ≥ 2.28 |
| 麒麟 V10 SP1 (2503/增强版)   | 2.31  | ✅ 支持  | x86_64 版本                       |
| UOS Server 20 (A版/Debian系) | 2.31  | ✅ 支持  | 内核 5.x 版本                     |
| UOS Server 20 (E版/欧拉版)   | 2.28  | ✅ 支持  | x86_64 版本                       |
...
2. **`backdrop-filter` 必须显式加 `pointer-events:none`**，且透明度为 0 时设为 `none`（不是 `blur(0px)`）：
...
3. **全屏遮罩容器设 `pointer-events:none`**，只给需要交互的子元素设 `pointer-events:auto`：
...
**原则**：少创建全屏 DOM 元素，每层都显式声明 `pointer-events`，`backdrop-filter` 不用就关掉。
...
**原因：** UE Development 构建会安装信号处理器（用于崩溃报告），这些处理器会被 fork 出的 CEF Helper 子进程继承，导致 GPU 进程启动异常，硬件视频解码失效。

| 构建配置           | linux_single_process | 视频/硬件解码                             |
| ------------------ | -------------------- | ----------------------------------------- |
